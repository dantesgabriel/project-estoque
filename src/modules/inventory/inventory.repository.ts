...
export const inventoryRepository = {
  findInProgress() {
    return prisma.inventory.findFirst({ where: { status: "IN_PROGRESS" } });
  },
  ...
  findItemById(inventoryId: string, itemId: string) {
  ...
  updateItemCount(itemId: string, countedQty: number, note: string | undefined, divergence: number) {
  ...
  async closeInventory(id: string, userId: string) {
    return prisma.$transaction(async (tx) => {
