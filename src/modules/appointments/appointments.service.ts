...
export const appointmentsService = {
  listResponsibleUsers() { return appointmentsRepository.listResponsibleUsers(); },
  ...
  async tutorHistory(tutorId: string) {
    const history = await appointmentsRepository.tutorHistory(tutorId);
    if (!history) throw new AppError("Tutor não encontrado", 404);
    return history;
  },
  async create(input: CreateAppointmentInput, createdById: string) {
    try {
      return await prisma.$transaction(async (tx) => {
        ...
        if (!tutor || !tutor.active) throw new AppError("Tutor não encontrado ou inativo", 404);
        if (!pet || !pet.active) throw new AppError("Pet não encontrado ou inativo", 404);
        if (pet.tutorId !== tutor.id) throw new AppError("O pet selecionado não pertence a este tutor", 400);
        if (!responsible || !responsible.active) throw new AppError("Responsável não encontrado ou inativo", 404);
        ...
        for (const item of input.items) {
          ...
          if (!product || !product.active) throw new AppError("Produto não encontrado ou inativo", 404);
          if (product.currentStock < item.quantity) {
            throw new AppError(`Estoque insuficiente para ${product.name}`, 409);
          }
          if (product.tracksBatch && !item.batchId) {
            throw new AppError(`Selecione o lote utilizado para ${product.name}`, 400);
          }

          let batchId: string | undefined;
          if (item.batchId) {
            const batch = await tx.batch.findUnique({ where: { id: item.batchId } });
            if (!batch || batch.productId !== product.id) throw new AppError(`Lote inválido para ${product.name}`, 400);
            if (batch.quantity < item.quantity) throw new AppError(`Saldo insuficiente no lote selecionado de ${product.name}`, 409);
      ...
      if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2034") {
        throw new AppError("O estoque foi atualizado simultaneamente. Tente registrar o atendimento novamente.", 409);
      }
      throw error;
    }
  },
  async cancel(id: string, reason: string, userId: string) {
    return prisma.$transaction(async (tx) => {
      ...
      if (!appointment) throw new AppError("Atendimento não encontrado", 404);
      if (appointment.status === "CANCELLED") throw new AppError("Este atendimento já foi cancelado", 400);
